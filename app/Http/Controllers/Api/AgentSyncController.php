...
class AgentSyncController extends Controller
{
        ...
        try {
            ...
            foreach ($accountsQuery->get() as $account) {
                ...
                $changes['accounts'][$status][] = [
                    ...
                    'balance'              => (float) $account->balance, // Référence absolue pour le mobile
            ...
            // 3. Synchronisation des Transactions (Inclut dépôts et retraits serveur/mobile)
            ...
            if ($lastSync) {
                ...
                $txQuery->latest()->limit(200); // Récupère les 200 dernières si premier sync
            }
    ...
    public function push(Request $request)
    {
        ...
        try {
            ...
            DB::commit();
            return response()->json(['success' => true, 'message' => 'Synchronisation réussie']);
    ...
    private function processPushClients(array $created, $agent): void
    {
        foreach ($created as $item) {
            ...
            // On stocke le mapping pour les comptes qui vont suivre
            $this->idMapping[$mobileUuid] = $client->id;
        }
    }
    ...
    private function processPushAccounts(array $accounts, $agent): void
    {
        foreach ($accounts as $item) {
            ...
            // Résolution de l'ID client (UUID local -> ID serveur)
            $realClientId = $this->idMapping[$mobileClientId] 
                ?? (is_numeric($mobileClientId) ? (int) $mobileClientId : null);

            if (!$realClientId) continue;

            // Éviter les doublons par numéro de compte
            $existing = Account::where('account_number', $item['account_number'])->first();
            if ($existing) {
                $this->idMapping[$mobileAccountId] = $existing->id;
                continue;
            }
            ...
            $account = Account::create([
                ...
                'balance'        => 0, // Le solde sera mis à jour par les transactions
                'status'         => 'active',
                'created_by'     => $agent->id,
                'activated_at'   => now(),
            ]);

            $this->idMapping[$mobileAccountId] = $account->id;

            // Initialisation Tontine si nécessaire
    ...
    private function processPushTransactions(array $transactions, $agent): void
    {
        foreach ($transactions as $item) {
            ...
            $realAccountId   = $this->idMapping[$mobileAccountId] 
            ...
            $type = $item['transaction_type'] ?? 'deposit'; // Défaut: dépôt

            // Anti-doublon (même jour, même compte, même type, même montant)
            $exists = Transaction::where('account_id', $realAccountId)
                ->where('amount', $amount)
                ->where('transaction_type', $type)
                ->whereDate('transaction_date', $serverNow->toDateString())
                ->exists();
    
            if ($exists) continue;
    
            // Gestion spécifique Tontine (Répartition en cycles)
            ...
            Transaction::create([
                'transaction_reference' => 'SYNC-' . strtoupper(Str::random(6)) . '-' . $serverNow->format('YmdHis'),
    ...
    // =========================================================================
    // LOGIQUE TONTINE (IDENTIQUE À L'ADMINISTRATEUR)
    // =========================================================================

    private function createTontineAccount(Account $account, array $item): ?TontineAccount
    {
        $tontineAmount       = (float) ($item['target_amount']         ?? 500);
    ...
    private function createTontineCycle(TontineAccount $tontine): ?TontineCycle
    {
        ...
        if ($cycleNumber === 1) {
            $startDate = Carbon::parse($tontine->cycle_start_date);
        } else {
            $lastCycle = $tontine->cycles()->latest('cycle_number')->first();
            $startDate = $lastCycle ? Carbon::parse($lastCycle->end_date)->addDay() : now();
        }
